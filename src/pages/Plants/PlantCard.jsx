import React from 'react'
import { API_URL } from '../../../.config'
import AnimatedButton from '../../components/Animations/AnimatedButton';
import { Link } from 'react-router';


const PlantCard = ({vegetable}) => {
  return (
   
    <div
        className="block rounded-lg bg-white p-6 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">
       <div className=''>
        <img src={`${API_URL}${vegetable.imageUrl}`} alt="" className='h-32 w-full object-cover'/>
        </div>
        <h5
          className="mb-2 text-xl font-medium leading-tight text-neutral-800 dark:text-neutral-50">
           {vegetable.commonName}
        </h5>
        <p className="mb-4 text-base text-neutral-600 dark:text-neutral-200">
          {vegetable.description}
        </p>
        <Link to={`/vegetables/${vegetable._id}`}>
                    <AnimatedButton text='More' type="orange" size="big" />
                  </Link>
          
        
      </div>
  )
}

export default PlantCard